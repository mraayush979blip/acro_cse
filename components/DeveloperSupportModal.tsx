...
const FEATURE_ROADMAP: Record<string, { status: string; eta: string; blurb: string }> = {
    ...
    "Developer Support": {
        status: "SUPPORT",
        eta: "NOW",
        blurb: "Support the continuous development of this app by buying the developer a coffee! Your contribution keeps this project alive and growing.",
    },
};
    ...
    return (
        <AnimatePresence>
            <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:px-4 bg-black/50 backdrop-blur-sm">
                ...
                >
                    <div className="bg-gradient-to-r from-indigo-800 to-indigo-600 p-6 text-center">
                        <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-3 backdrop-blur-md">
                            <Coffee className="w-8 h-8 text-white" />
                        </div>
                        <h2 className="text-xl font-bold text-white">
                            {featureName === "Developer Support" ? "Encourage Developer" : "Work in progress"}
                        </h2>
                    ...
                    <div className="p-6 text-center">
                        {showQR ? (
                            <div className="flex flex-col items-center animate-in fade-in zoom-in duration-300">
                                <p className="text-sm font-bold text-slate-800 mb-4">Scan QR Code to Support</p>
                                <div className="w-[260px] h-[300px] rounded-xl shadow-lg border border-slate-200 mb-6 overflow-hidden bg-black relative mx-auto">
                                    <img src="/qr_code.jpg" alt="UPI QR Code" className="absolute top-1/2 left-1/2 w-[145%] max-w-none h-auto -translate-x-1/2 -translate-y-[47%]" />
                                </div>
                                <button
                            ...
                            <>
                                <div className="flex justify-center gap-2 mb-3 text-xs font-bold uppercase tracking-wide">
                                    <span className="px-2 py-1 rounded-full bg-indigo-50 text-indigo-700">{info.status}</span>
                                    <span className="px-2 py-1 rounded-full bg-slate-100 text-slate-600">{info.eta}</span>
                                </div>
                                ...
                                <div className="flex flex-col space-y-3">
                                    <button
                                        ...
                                        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-bold shadow-md shadow-indigo-600/30 active:scale-[0.98] transition-all flex items-center justify-center"
                                    >
                                        <Coffee className="w-5 h-5 mr-2" />
                                        {featureName === "Developer Support" ? "Buy a coffee to support developer" : "Buy a coffee for faster shipping"}
                                    </button>
                                    <button
                                        onClick={onClose}
                                        className="w-full bg-gray-100 hover:bg-gray-200 text-[#54656f] py-3 rounded-xl font-bold active:scale-[0.98] transition-all"
